...
func TestLoadConfig_InsecureSkipVerify(t *testing.T) {
	// Removed in v1.5: TLS skip-verify is no longer a Technitium-specific
	// concern. The unified DNSWEAVER_{NAME}_TLS_SKIP_VERIFY (and legacy
	// _INSECURE_SKIP_VERIFY alias) is now consumed by the provider registry
	// and applied at the shared httputil layer. See pkg/httputil and
	// pkg/provider tests for coverage.
	t.Skip("TLS configuration moved to unified pkg/httputil + pkg/provider layer")
}

func TestLoadConfigFromMap_InsecureSkipVerify(t *testing.T) {
	// Removed in v1.5: see TestLoadConfig_InsecureSkipVerify.
	t.Skip("TLS configuration moved to unified pkg/httputil + pkg/provider layer")
}
